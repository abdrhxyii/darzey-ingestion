...
Backfill workflows process source records in bounded batches and keep a
...
The checkpoint records the next source page or batch. It advances only after
the preceding batch has fully succeeded, so interrupted workflows can resume
safely. Source-specific discovery and storage details are documented with the
corresponding connector and workflow.
