"""Backfill official Acts archive years into private R2 storage."""
...
from legalai_ingestion.connectors.documents_gov_lk.acts_archive import discover_acts_for_year_range
...
def main() -> int:
    ...
    print(f"Backfilling Acts published from {arguments.from_year} to {arguments.to_year}...")
    total = store_documents(
        discover_acts_for_year_range(arguments.from_year, arguments.to_year),
        store=store,
    ...
    print(
        f"Acts backfill complete: {total.checked} checked; {total.pdfs_uploaded} PDFs uploaded; "
