"""Backfill official Gazette archive years into private R2 storage."""
...
from legalai_ingestion.connectors.documents_gov_lk_gazettes_archive import discover_gazettes_for_year_range
...
def main() -> int:
    ...
    total = store_documents(discover_gazettes_for_year_range(arguments.from_year, arguments.to_year), store=store,
                            download_pdf=download_pdf, pipeline_version="0.6.0",
                            minimum_download_interval_seconds=arguments.minimum_download_interval_seconds)
    print(f"Gazette backfill complete: {total.checked} checked; {total.pdfs_uploaded} PDFs uploaded; "
